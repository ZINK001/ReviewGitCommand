...
```
^与~之间的区别，当存在多个分支时，^可以用来选择分支，
HEAD~i永远只选择第i级父节点的第一个分支，HEAD~i^2选择第i级父节点的第二个分支，以此类推；
HEAD^=HEAD^1=HEAD~1；如果没有分支，只有一条主线，则HEAD^^^=HEAD^1^1^1=HEAD~3，
...
### Rebase修改历史提交记录（提交作者（邮箱）和提交信息）
```
$ git rebase -i commit_id // 要修改信息的前一个commit_id
// 进入vim编辑界面
// r:修改提交信息 e:修改提交作者
$ git commit --amend --author="fulibo<fulibowork@163.com>"  --no-edit
$ git rebase --continue
```
...
```
# pick
> 只将develop中某几个commit合到master中
```
$ git cherry-pick 38361a55
// 出现冲突 解决冲突
$ git add readme.md
$ git commit -c 15a2b6c6  // 新的commit_id 提示中会有
...
$ git cherry-pick <start-commit-id>..<end-commit-id>  // 不包含start
$ git cherry-pick <start-commit-id>^..<end-commit-id> // 包含start
```
